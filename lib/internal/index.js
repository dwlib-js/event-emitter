...
const EventEmitterEvents = eventEmitter => {
  ...
  let iteratorResult = MapIteratorNext(iterator);
  while (!iteratorResult.done) {
    events[index++] = iteratorResult.value;
    iteratorResult = MapIteratorNext(iterator);
  }
  return events;
}
...
const EventEmitterSubscribe = (eventEmitter, event, listener, options) => {
...
const IsEventEmitter = argument => (
  IsObject(argument) && HasInternalSlot(argument, $Listeners)
);
...
class EventEmitter {
  ...
  addListener(event, listener, options) {
    EventEmitterAddListener(this, event, listener, options);
    return this;
  }
  ...
  subscribe(event, listener, options) {
    return EventEmitterSubscribe(this, event, listener, options);
  }
}
ReflectDefineProperty(EventEmitter, SymbolHasInstance, {
